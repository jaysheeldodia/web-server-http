...
void signal_handler(int signal) {
    ...
    // Don't call cleanup directly from signal handler - too risky
...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        // Only enable TLS if certificate files exist and if we're testing
        bool enable_tls_for_testing = false; // Set to false for basic testing
        if (enable_tls_for_testing) {
            std::ifstream cert_check("config/cert.pem");
            std::ifstream key_check("config/key.pem");
            if (cert_check.good() && key_check.good()) {
                server.enable_tls(true, "config/cert.pem", "config/key.pem");
                std::cout << "\n=== TLS/HTTPS Enabled ===" << std::endl;
                std::cout << "✓ Certificate: config/cert.pem" << std::endl;
                std::cout << "✓ Private Key: config/key.pem" << std::endl;
            } else {
                std::cout << "\n=== TLS/HTTPS Disabled ===" << std::endl;
                std::cout << "! Certificate files not found in config/ directory" << std::endl;
                std::cout << "! Server will run in HTTP-only mode" << std::endl;
            }
            cert_check.close();
            key_check.close();
        } else {
            std::cout << "\n=== TLS/HTTPS Disabled for Testing ===" << std::endl;
            std::cout << "! Running in HTTP-only mode for basic functionality test" << std::endl;
        }
